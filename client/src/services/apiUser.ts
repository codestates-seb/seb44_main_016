...
const apiUser = {
  ...
  postOAuthCode: async (OAuthReqData: OAuthReqData) => {
    const response = await axios.post(
      `/login/oauth2/code/${OAuthReqData.targetOAuth}`,
  ...
  deleteMyInfo: async (password: string) => {
    const response = await instance.delete(`/user/delete`, { data: { password: password } });
  ...
  getMyFeeds: async (userId: number, page: number, size: number) => {
    ...
    const res = await instance.get(`/user/${userId}/feedArticles?page=${page}&size=${size}`);
  ...
  updateMyInfo: async (userUpdateData: FormData) => {
    const response = await instance.patch(`/user/update`, userUpdateData, {
