...
export const APIfinancialRecord = {
  // 가계부 전체조회 GET
  getRecordList: async () => {
    const res = await instance.get(`/financial-record`, {
  ...
  createFaRec: async (formData: FormData) => {
    ...
    const res = await instance.post(`/financial-record`, formData, {
      headers: {
  ...
  getFaRec: async (financialRecordId: number) => {
    const res = await instance.get(`/financial-record/${financialRecordId}`);
  ...
  updateFaRec: async (formData: FormData, financialRecordId: number) => {
    ...
    const res = await instance.patch(`/financial-record/${financialRecordId}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  getRecordArticle: async (financialRecordId: number, page: number, size: number) => {
    ...
    const res = await instance.get(
      `/financial-record/${financialRecordId}/article?page=${page}&size=${size}`
    );
